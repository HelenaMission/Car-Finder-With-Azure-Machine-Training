  ...
  const handleUploadClick = async () => {
    if (uploadedImage) {
      try {
        ...
        const response = await axios.post("http://localhost:4000/car_Finder", formData);

        // Find out the car model tagname by using reduce function
        const carMakePredic = response.data.carMakePredic;
        const highestProbabilityPrediction = carMakePredic.predictions.reduce(
          (highest, prediction) => (prediction.probability > highest.probability ? prediction : highest),
          { probability: 0 }
        );

        setCarModel(highestProbabilityPrediction.tagName);

        // Find out the car make tagname by using reduce function
